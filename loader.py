    ...
    with codecs.open(filename,'r',encoding='utf-8') as f:
        for line in f:
            try:
                ...
                '''
                contents.append((content.encode('utf-8').decode('utf-8-sig')).split(' '))
            except:
                pass
...
def read_category():
    ...
    categories = ['__label__0','__label__1','__label__-1']
...
def process_file(filename,word_to_id,cat_to_id,max_length=600):
    ...
    return x_pad,y_pad,contents

def batch_iter(x,y,x_train_topic,batch_size=64):
    """
    Args:
    ...
    x_shuffle=x[indices]
    y_shuffle=y[indices]
    x_train_topics=x_train_topic[indices]
    for i in range(num_batch):
        start_id=i*batch_size
        end_id=min((i+1)*batch_size,data_len)
        yield x_shuffle[start_id:end_id],y_shuffle[start_id:end_id],x_train_topics[start_id:end_id]
...
def get_training_LDA_vectors(filename):
    """
    Args:
        filename:numpy file
    Returns:
